def BLACKJACK():
    ...
    class Account:
        def __init__(self,owner,balance):
            ...
            print ("Hi {owner}, welcome to Banco De Maretima! You have a balance of {balance}".format)


        def dep(self,amt):
            self.balance = self.balance + amt
            print("Thanks for your deposit of {amt}, {self.owner}.Your current balance is {self.balance}".format)

        def withd(self,amt):
            if amt < self.balance:
                self.balance = self.balance - amt
                print ("Hi {self.owner}. You have made a withdrawal of {amt}, your new balance is {self.balance}".format)
            else:
                ...
                while True:
                    try:
                        ...
                        if betmore > 0 and betmore < playerBal:
                            ...
                            print('You have now placed a ${bets} bet'.format)
                            p1stay()
                            Dcarddeal()
                            break
                        elif betmore == 0:
                            p1stay()
                            print('Your bet remains at ${bets}.'.format)
                            Dcarddeal()
                            break
                        else:
    ...
    def carddeal():
        ...
        print('{len(deck)} cards remain in deck{decknum}'.format)
        print('{pick} is the random intgr.'.format)
        ...
        if(deck[pick][0]) == "A":
            ...
            try:
                ...
                print('no v')
        print('{playerName},You have {p1} that is {sum(p1value)} for {playerName}'.format)
        deck.pop(pick)
        print('That card was picked from deck - {decknum}'.format)    
        print('Dealer has{dealer} that is {sum(dealervalue)} for the Dealer'.format)
        wincheck()
        ...
        while True:
            try:
                ...
                if p1bet > 99 and p1bet < playerBal:
                    ...
                    print("{playerName} has placed a bet of {p1bet}".format)
                    print(playerBal)
                    carddealStart()
                    break
                else:
